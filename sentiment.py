...
def sentiment_output(input_file, data_file):
	health_words = set()
	print('\nSentiment Reading in CSV...')

	df_clusters_all = pd.read_csv(join(input_file,'clusters.csv'), header=None)
	#df_clusters_all = pd.read_csv('clusters.csv', header = 0)
	df_clusters_health = df_clusters_all.loc[df_clusters_all[0] == 1]
	#print(df_clusters_health)
	df_clusters_health_nolabel = df_clusters_health.drop(0, axis=1)
	#print(df_clusters_health_nolabel)
	...
	print('Finished Reading, assigning sentiment to reviews... (lengthy)')
	...
	print('Assigning scores to businesses...')
	...
	with open(join(input_file,'health_scored_busineses.csv'), 'w') as f:
		w = csv.writer(f)
